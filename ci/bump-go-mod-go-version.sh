...
go_toolset_tag_exists() {
  ...
  if command -v docker >/dev/null 2>&1; then
    if timeout 30 docker manifest inspect "${image_ref}" >/dev/null 2>&1; then
  ...
  if command -v podman >/dev/null 2>&1; then
    if timeout 30 podman manifest inspect "docker://${image_ref}" >/dev/null 2>&1; then
      echo "go-toolset image tag is available: ${image_ref}"
      return 0
    fi
  fi
...
release_json="$(curl -fsSL --connect-timeout 10 --max-time 30 "${GO_RELEASES_URL}")"
updated_any="false"
